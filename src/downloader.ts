...
import { FePluginError, getLogger } from "./util";
...
export class CompilerDownloader {
  ...
  public get downloadsDir(): string {
    return path.join(this._compilersDir, "fe", this._platform);
  }
  ...
  public async getCompilerAsset(
    ...
    if (versionRelease === undefined) {
      ...
      if (versionRelease === undefined) {
        throw new FePluginError(`Unsupported fe version: ${version}`);
      }
    }
    ...
    if (compilerAsset === undefined) {
      throw new FePluginError(
        `Fe version ${version} is not supported on platform ${this._platform}`
      );
    }
  ...
  public async getOrDownloadCompiler(
    ...
    try {
      ...
      if (FePluginError.isNomicLabsHardhatPluginError(e)) {
        throw e;
      } else {
        throw new FePluginError(
  ...
  private async _downloadCompiler(
    ...
    try {
      ...
      throw new FePluginError("Compiler download failed", e as Error);
    }
  }
  ...
  private _getCurrentPlatform(): CompilerPlatform {
    switch (os.platform()) {
      ...
      default:
        throw new FePluginError(
          `Fe not supported on platform ${os.platform()}`
        );
    }
  }
}
